...
COMMENT ON TABLE university_connection IS
...
COMMENT ON COLUMN university_connection.id IS
'Primary key (UUID) generated via gen_random_uuid().';

COMMENT ON COLUMN university_connection.user_id IS
'Reference to omu_user.id. Defines ownership of the connection.';

COMMENT ON COLUMN university_connection.university_id IS
'Short university identifier (e.g. UNIMOL, UNIROMA1). Matches Cineca tenant identifier.';

COMMENT ON COLUMN university_connection.university_name IS
'Human-readable name of the university.';

COMMENT ON COLUMN university_connection.cineca_base_url IS
'Base URL of the ESSE3 REST API instance for the university.';

COMMENT ON COLUMN university_connection.username_cineca IS
'Cineca ESSE3 username used for authentication at this university instance.';

COMMENT ON COLUMN university_connection.connected_at IS
'Timestamp when the connection was created.';

COMMENT ON COLUMN university_connection.last_used_at IS
