...
public abstract class AbstractEsse3Controller {
   ...
   * {@link CinecaBookingException} to {@code 422 Unprocessable Entity} with the
   * raw Cineca message so it can be shown to the user.
   *
   * @param principal the authenticated OhMyU user (used for logging)
   * @param action    runnable performing the booking/cancellation
   * @return {@code 204 No Content} on success, {@code 422} with the Cineca
   *         message if rejected, {@code 401} if the session expired, or
   *         {@code 503} if Cineca is unreachable
   */
